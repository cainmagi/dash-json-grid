...
| `*args, **kwargs` | `...` |  <InlineIcon icon={biSlashLg}/>   | The same as the initialization, see [component properties](#component-properties). The exception is `data` which should not be configured because it is replaced by `json_string`. |
