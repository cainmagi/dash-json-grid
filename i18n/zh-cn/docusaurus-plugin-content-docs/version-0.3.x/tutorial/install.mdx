...
在开发阶段，GitHub上的版本有可能比在PyPI发布的版本新。如果用户需要安装GitHub版本，请确保已经
安装了以下软件：
...
若有需要，可以在`pip install`命令的后面加上`-e`。这将令所安装的模块支持即时修改。亦即，
对从GitHub上clone的项目做出的任何改动都将反馈到全局安装的`dash_json_grid`模块包上。
...
浏览器必须使用Chrome或者Chromium。Edge或者Opera无法与
[`dash_duo`<IconExternalLink/>][link-dash-duo]兼容。
