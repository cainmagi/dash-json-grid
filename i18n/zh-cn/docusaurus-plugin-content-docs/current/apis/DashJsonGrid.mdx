...
`DashJsonGrid`组件是对React项目
[`react-json-grid/JSONGrid`<IconExternalLink/>][link-jsongrid]的Dash移植版。
...
| `route` | [`Route`](./mixins/Route.mdx) | <InlineIcon icon={vsiCheckIcon}/> | 用来定位`data`数据的某一特定部分的路由，由数据各级下标构成。末位元素可能是一个单元素序列，以用来表示被选中的区域是表格、或表格的列。 |
...
使用`dash_json_grid<=0.3.2`时，运行该范例会触发`KeyError: 'key3'`。因为`0.3.3`版之前，
该方法只能删除存在于原数据之中的关键字。
