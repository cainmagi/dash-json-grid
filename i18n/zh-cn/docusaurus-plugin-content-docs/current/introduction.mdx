...
注意此处的范例纯粹是用React JSON Grid写就。Dash JSON Grid仅仅是React JSON Grid的封装，因此可以认为Dash JSON Grid能实现和以下范例相同的效果。然而，由于以下范例是由文档的全局依赖项<AppVer/>构建，其与旧版的Dash JSON Grid所依赖的版本可能不同，故而，该范例未必能确切地和你正在检索的Dash JSON Grid版本对应上。
