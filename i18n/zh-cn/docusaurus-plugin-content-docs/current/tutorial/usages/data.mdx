...
### 展开数据的一部分 {#expand-a-part-of-the-data}

处理形如上例中的复杂`data`属性时，有可能需要默认展开其中的一小部分。下例展示了如何透过属性`default_expand_key_tree`设置这种部分展开的特性：
...
<ViewOnlyDemoApp
  ...
  defaultExpandKeyTree={{"not-a-table": {"2": true}}}
  theme="default"
/>
...
注意`default_expand_key_tree`的叶子节点需要是二值量。设置为`True`的叶子节点，将展开对应的路由。
...
[link-sequence]: https://docs.python.org/zh-cn/3/library/stdtypes.html#sequence-types-list-tuple-range
[link-mapping]: https://docs.python.org/zh-cn/3/library/stdtypes.html#mapping-types-dict
[link-union]: https://docs.python.org/zh-cn/3/library/stdtypes.html#frozenset.union
