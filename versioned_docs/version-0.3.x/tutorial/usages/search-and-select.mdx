...
description: A guide showing how to fed the data query and how to collect the selected value.
...
The following example uses a [`dcc.Input`<IconExternalLink/>][link-dccinput] component
to specify the keyword (query) used for searching the data. All values that are
searched will be highlighted with a special style.
...
`search_text` is a set-only property. It means that this property can be only
...
`highlight_selected` is a set-only property. It means that this property can be only
set by a callback. The component itself will not fire a callback with an output of
...
`selected_path` is an interact-only property. It means that user interactions can
change this property and fire a callback. However, configuring this property by
...
a [`dash.Output`<IconExternalLink/>][link-dashoutput].
